...
**Documentation, not the code, becomes the primary artifact.** Documentation is the source that generates implementation. Code is an expression of that documentation in a particular language and framework — not the other way around. This moves the lingua franca of development to a higher level, making the work accessible to anyone who can read the documents, not just those fluent in the codebase.
